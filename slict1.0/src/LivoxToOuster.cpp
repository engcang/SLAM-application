...
public:
    ...
    LivoxToOuster(ros::NodeHandlePtr &nh_ptr_) : nh_ptr(nh_ptr_)
    {
        ...
        livoxCloudSub = nh_ptr->subscribe<livox_ros_driver::CustomMsg>("/livox/mid360", 50, &LivoxToOuster::cloudHandler, this, ros::TransportHints().tcpNoDelay());
        ousterCloudPub = nh_ptr->advertise<sensor_msgs::PointCloud2>("/livox/lidar_ouster1", 50);
    }
